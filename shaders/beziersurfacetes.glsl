...
layout( quads ) in;
out vec4 FragNormal; // Vertex normal in camera coords.
out vec4 FragPosition; // Vertex position in camera coords
uniform mat4 model;
uniform mat4 view;
...
void main()
{
    ...
    FragPosition = view * model * FragPosition;
    FragNormal = normalize(inverse(transpose(model)) * vec4(n, 0.0));
}
